...
| 7 | Generated live learning project: one `create/init` implementation produces a standalone slots-booking project with the real app, real Checkly check, deployment config, CI gate, an empty `incidents/`, and a guided setup/cleanup README. The learner owns every external resource and captures a fresh incident. | the packed generator creates the project outside this repo; a learner completes GitHub → Vercel → Upstash → Checkly → incident → bundle → repair → PR gate with no mock inside the project | blocked on Phase 6 |

## Phase 7 — Generated live learning project

Phase 7 starts only after Phase 6 is complete.

| Task | Technical detail |
| --- | --- |
| 7.1 Create/init command | `npm create verify-fix@latest <dir> -- --template slots-booking-live` invokes a separate `create-verify-fix` package. If the installed CLI also exposes `verify-fix init`, it must call the same generator. The command checks Node, refuses a non-empty destination, supports non-interactive and no-install modes, and never reads or stores credentials. |
| 7.2 Standalone real template | Assemble the template from the maintained `examples/slots-booking/web` source. Generate the real Next.js app, real Playwright and Checkly configuration, provider-specific Vercel example config, GitHub Actions gate, unique Checkly project identifiers, an empty `incidents/`, and a short README. Do not include a captured bundle, fake executable, stub API, fabricated result, hardcoded verdict, `node_modules`, `.next`, report, raw trace, or `.env` file. |
| 7.3 Learner-owned setup | The README guides the learner through creating a GitHub repository, deploying to Vercel, attaching Upstash, authenticating Checkly, deploying the scheduled check, and configuring protected GitHub environments. The generator does not provision or manage any service. Credentials stay in the learner's environment. Rocky Automatic Repair remains off. |
| 7.4 Fresh incident and full loop | The two real Checkly locations use the same test account against the shared Upstash store. Their collision creates the learner's real incident. The learner runs `verify-fix bundle` to generate the first contents of `incidents/`, creates a repair branch, verifies against the exact preview URL and revision, opens a pull request, and lets the CI gate decide before `checkly deploy`. |
| 7.5 Package and safety proof | Pack both npm packages. Run the packed creator from a temporary directory outside this repo. Install the generated project. Confirm it has no repository-relative imports. Run its real Next.js app, real Chromium, real Playwright, and real `verify-fix` locally. The account-backed workshop is then completed from the user's machine. Document Checkly run usage, possible alerts, unique logical IDs, failure recovery, and cleanup through Checkly, Vercel, Upstash, and GitHub. |

**Done when:** a project produced by the packed create/init command completes the
full learner-owned flow on real accounts. The generated project starts with no
...
revision. No mock, secret, or repository-relative dependency exists inside the
generated project.
