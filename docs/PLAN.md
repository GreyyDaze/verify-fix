...
| 2b | Second incident on the same app: **drift**. Product renames `data-testid="book-status"` → `booking-status` (`app/book/page.tsx`); the check goes stale and fails at spec line 36 in every location; Rocky writes an RCA with a `codeFix` — the first real LLM-written check patch in this repo. Capture with `bundle --out fixtures/bundles/slots-booking-drift`, golden test. Known gaps the capture will show: no failing request in the recording → detection scene `inject:<failing request unknown>` (needs assertion → network dependency from the passing recording); `failing`/`passing` naming assumes the app broke, in drift the *check* is stale. Both are Phase 3 work | fixture with a non-null `codeFix` committed | rename committed; deploy + capture on the customer side pending |
