...
test.describe('slots booking flow', () => {
  test('log in and book the 09:30 slot', async ({ page }) => {
    ...
    await expect(page.getByTestId('booking-status')).toHaveText('200', { timeout: 200 }).catch(() => {})

    // 4. booking confirmed
    await expect(page.getByTestId('booking-result')).toHaveText('CONFIRMED', { timeout: 200 }).catch(() => {})
  })
})
