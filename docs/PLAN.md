...
├── examples/slots-api-express/ Example 2 (Phase 7): Express API + Checkly ApiCheck
...
| 6 | Candidate revision intake: replace the production idea of one agent patch with the complete final project state. Support both a local working tree and a pull request URL. Treat every agent change as untrusted. Bind PR verification to the exact preview revision. | the same multi-file candidate is verified from a fixed local snapshot and from an exact PR head; reports record source identity and all file changes; the candidate cannot replace the verifier, incident, policy, or gate | **local implementation done** — immutable dirty-tree and exact GitHub PR snapshots, stable logical-ID matching, rename/deletion/import handling, complete source reports, target metadata binding, fork/approval controls, protected two-stage workflow, package proof, and 123/123 tests. Real protected preview + Checkly proof remains blocked on the Phase 5 account checkpoint. |
| 7 | Real Express API + Checkly `ApiCheck`: Railway production and PR environments, `{{ENVIRONMENT_URL}}` with no fallback, a real setup script, a changed-response incident captured from the learner's account, API-result bundling, deterministic replay/injection scenes, local/cloud parity, complete candidate projects, and a Railway CI adapter | real Railway deployment and Checkly check produce a fresh field-rename incident; strict direct and multi-file monitoring repairs PASS; weakening, redirect, setup-script, and retry-only candidates FAIL; missing handlebars input is UNCERTAIN; a real Railway PR preview passes the protected gate | blocked on Phase 6 |
...
## Phase 7 — Real Express API and Checkly ApiCheck

Phase 7 starts only after Phase 6 is complete.

| Task | Technical detail |
| --- | --- |
| 7.1 Real API project | Create `examples/slots-api-express/` as one TypeScript project containing the Express service and its Checkly Monitoring as Code files. Expose `/health` and an authenticated `/v1/profile` endpoint. Bind to `0.0.0.0` and Railway's `PORT`. Keep app code, check code, package manifest, lockfile, deployment config, and README together. Use no simulator or stub server. |
| 7.2 Real Checkly ApiCheck | Define a stable `ApiCheck` logical ID. Request `{{ENVIRONMENT_URL}}/v1/profile` with no JavaScript fallback. Use a real setup script that reads a preview-only `API_TOKEN`, adds the Authorization header, and adds a request ID. Assert status `200`, JSON content type, and the exact JSON property/value contract. Store values in the user's environment and Checkly. Never commit them. Current Checkly documentation confirms that API-check handlebars need remote variables and have no fallback. |
| 7.3 Baseline and fresh incident | Deploy the baseline API to Railway. Deploy the check to Checkly. Keep it green long enough to capture passing history. Then rename the response field from `displayName` to `name` without changing the check. Let the scheduled Checkly check fail against the real Railway service. Capture the real result, request, response, history, assets, and RCA with `verify-fix bundle`. Record what Checkly actually returns. Do not assume the RCA classification in advance. |
| 7.4 API bundle support | Extend bundle capture for Checkly API-check result shapes. Build a sanitized HAR-equivalent request/response record when no Playwright trace exists. Preserve method, path, query, safe headers, setup-script provenance, status, content type, and the JSON contract evidence. Remove authorization values, cookies, tokens, account data, and configured secret values before writing. Keep the real captured bundle under `fixtures/bundles/slots-api-express-response-change/` with a golden test. |
| 7.5 Deterministic API scenes | Add an API scene executor for the declarative `ApiCheck` request and supported `AssertionBuilder` operations. Run the real setup script in a restricted sandbox before the request. Resolve only the request origin through the supplied target or scene proxy. Preserve path, query, method, body, and safe headers. Use live HEALTHY evidence against the candidate target. Use the captured changed response for replay REPRODUCTION. Add a DETECTION response that violates the repaired field contract. Missing `ENVIRONMENT_URL` produces UNCERTAIN. An unsupported Checkly feature produces UNCERTAIN instead of guessed behavior. Validate supported behavior against real `checkly test` sessions. |
| 7.6 Repair and rejection matrix | Test complete candidate project states instead of assuming one changed file. One strict monitoring repair updates the JSON assertion to the new field. A second strict repair moves the exact field contract into an imported helper and updates the check across multiple files. Reject removal of the JSON assertion, status-only checks, weaker matchers, a switch to `/health`, a hardcoded production host, `shouldFail` inversion, response rewriting in setup or teardown code, retry-only changes, and timeout-only changes. This replay-focused incident proves monitoring repairs. It does not claim that replay can validate an application-only repair that changes the response itself. Keep assertion identity, the scene decision table, and mutation rules unchanged. |
| 7.7 Railway preview gate | Use Railway as the second real deployment provider. Enable Railway's native PR environments manually. Do not provision Railway from verify-fix. Read the actual preview URL and deployed commit from provider or GitHub deployment metadata. Never guess the URL from a naming pattern. Pass that URL and revision into the provider-independent verify-fix core. Run candidate HEALTHY/REGRESSION through the project-local Checkly CLI. Run replay and detection through the local API scene executor. Deploy monitoring source only after PASS. Document that bot-created PR environments require an explicit Railway setting. Clean up PR environments when the PR closes. |
| 7.8 Proof and cost report | Run the app and supported API scenes locally with real Node HTTP traffic. Pack verify-fix and invoke it from the external Express project. Then run a real Railway PR preview and real Checkly sessions from the user's machine and account. Record local runs, Checkly cloud runs, mutation runs, completed requests, and wall time per verdict. Update the project guide with the input, data flow, output, credentials, costs, provider boundary, failure recovery, and cleanup steps. |

### Important Phase 7 cases

- **Handlebars target.** `{{ENVIRONMENT_URL}}` has no fallback. A missing value is UNCERTAIN. A candidate must not replace it with a hardcoded host.
- **Host-only replacement.** Change only the request origin for preview, replay, and injection. Preserve the route, query, method, body, and contract headers.
- **Setup-script failure.** A thrown setup error stops the request. Report UNCERTAIN when execution evidence is missing. Do not call that a check failure.
- **Setup and teardown suppression.** Reject candidate code that rewrites the response or hides the failed assertion.
- **Secret handling.** Never write the API token, Authorization header, Checkly key, Railway token, or secret environment value into a bundle or report.
- **API result shape.** Learn the current Checkly response format from the real capture. Do not copy assumptions from Playwright results.
- **Response encoding.** Preserve valid JSON evidence. Fail closed on truncated, compressed, binary, or malformed bodies that cannot prove the assertion result.
- **Assertion parity.** Implement only operators confirmed against Checkly. Return UNCERTAIN for unsupported operators.
- **Assertion identity.** Parse `AssertionBuilder` calls and statically resolved imported constants into the existing inventory. Keep the current `fnv1a("matcher|target")` identity law unchanged. Return UNCERTAIN when the target cannot be resolved safely.
- **Retry policy.** Retries can measure recurrence. Retries cannot turn an unchanged broken contract into a repair.
- **Candidate scope.** Support check-only, config-only, setup-helper, dependency, and combined multi-file monitoring candidates. Do not claim application-only replay proof when the application change would alter the captured response.
- **Preview identity.** Require the Railway preview commit to equal the candidate commit before an authoritative verdict.
- **Bot PR safety.** Railway does not deploy bot PR environments unless the owner enables that behavior. Keep the setting off unless the learner deliberately enables it.
- **Provider boundary.** Railway supplies a URL and revision. No Railway API belongs in the verify-fix decision engine.
- **Real evidence only.** Use the real Express app, real Railway deployment, real Checkly ApiCheck, real Checkly CLI, real setup script, real request, real response, and real captured incident.

Public behavior checked on 2026-09-24: [Checkly API-check environment variables](https://www.checklyhq.com/docs/cli/environment-variables/), [ApiCheck construct](https://www.checklyhq.com/docs/constructs/api-check/), [setup and teardown execution](https://www.checklyhq.com/docs/detect/synthetic-monitoring/api-checks/setup-and-teardown/), and [Railway native PR environments](https://docs.railway.com/guides/preview-deployments-with-pr-environments).

**Done when:** the Express API is deployed to Railway. The Checkly ApiCheck is
green before the response change. The real field rename creates a fresh failing
result. `verify-fix bundle` captures and sanitizes that incident. The supported
local executor matches real Checkly outcomes. The direct and multi-file strict
monitoring repairs PASS. Every weakening and masking candidate FAILS. Missing target input is UNCERTAIN. A real Railway PR preview is bound to
its exact revision and passes the gate. Reports include complete cost evidence.
No fake service, result, incident, or verdict is used inside the example.
