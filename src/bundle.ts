...
export function loadBundle(dir: string): LoadedBundle {
  ...
  if (raw.schemaVersion === "v3") {
    throw new Error(
      `${manifestPath} is a v3 bundle generated by \`verify-fix bundle\`. ` +
        "`verify` consumes v3 bundles from Phase 3 (generic scene layer); until then use a v2 bundle directory.",
    );
  }
