...
test("bundle: end to end with a fake Checkly — files, sanitized HAR, manifest, no secrets", async () => {
  ...
  try {
    ...
    // the verifier loads the generated bundle: scenes carry runnable modes, config comes from the API document
    const { bundle } = loadBundle(out);
    assert.equal(bundle.schemaVersion, "v3");
    assert.equal(bundle.dir, out);
    assert.equal(bundle.check.file, manifest.check.file);
    assert.equal(bundle.checkSource.length > 0, true);
    assert.deepEqual(bundle.config?.environmentVariables, manifest.config.environmentVariables.map((e: { key: string }) => e.key), "keys only, never values");
    assert.deepEqual(bundle.scenes.map((s) => s.mode), manifest.scenes.map((s: { mode: string }) => s.mode));
    assert.equal(bundle.determinism.achieved, 0, "unmeasured determinism keeps the PR-7 gate closed");
    assert.equal(bundle.files["checkly.config.ts"] !== undefined, true);
